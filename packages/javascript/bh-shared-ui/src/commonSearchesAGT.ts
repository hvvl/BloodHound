...
export const CommonSearches: CommonSearchType[] = [
    ...
    {
        subheader: 'Kerberos 交互',
        category: categoryAD,
        queries: [
            {
                name: 'Tier Zero / 高价值组中可 Kerberoast 的成员',
                description: '',
                query: `MATCH (u:User)\nWHERE (u:${TAG_TIER_ZERO_AGT}) AND u.hasspn=true\nAND u.enabled = true\nAND NOT u.objectid ENDS WITH '-502'\nAND NOT COALESCE(u.gmsa, false) = true\nAND NOT COALESCE(u.msa, false) = true \nRETURN u\nLIMIT 100`,
            },
            {
    ...
    {
        subheader: 'Active Directory 证书服务',
        category: categoryAD,
        queries: [
            {
                name: 'PKI 层级结构',
...
RETURN p\nLIMIT 1000`,
    ...
    {
        subheader: 'Active Directory 安全卫生',
        category: categoryAD,
        queries: [
            {
    ...
    },
    {
        ...
        queries: [
            ...
            {
                name: '正在运行 WebClient 的计算机',
                description: '',
                query: `MATCH (c:Computer)\nWHERE c.webclientrunning = True\nRETURN c LIMIT 1000`,
            },
            {
                name: '不要求入站 SMB 签名的计算机',
                description: '',
